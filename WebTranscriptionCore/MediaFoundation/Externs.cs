...
using Microsoft.MediaFoundation.EVR;
using Microsoft.MediaFoundation.MFPlayer;
...
namespace Microsoft.MediaFoundation
{
	public static class MFExtern
	{
		...
		{
			if (pAttributes.GetDouble(guidKey, out double fRet) < 0)
			{
				fRet = fDefault;
			}
			return fRet;
		...
		{
			if (pAttributes.GetUINT32(guidKey, out int unRet) < 0)
			{
				unRet = unDefault;
			}
			return unRet;
			 ...
			 )
		{
			...
			hr = pAttributes.GetUINT64(guidKey, out long unPacked);
			if (hr < 0)
			{
		...
		{
			if (pAttributes.GetUINT64(guidKey, out long unRet) < 0)
			{
				unRet = unDefault;
			}
			return unRet;
			 ...
			 )
		{
			...
			int hr = pAttributes.GetStringLength(guidKey, out int length);
			if (hr >= 0)
			{
		...
		public static void StartMF()
		{
			int hr = 0;
			if (referenceCounter <= 0)
			{
				hr = MFExtern.MFStartup(0x10070, 0);
				Marshal.ThrowExceptionForHR(hr);
			}
			referenceCounter++;
		}

		public static void StopMF(Boolean force = false)
		{
			referenceCounter--;
			if (referenceCounter > 0 && !force) return;
			MFExtern.MFShutdown();
