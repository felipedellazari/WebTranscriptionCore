...
namespace WebTranscriptionCore {
	public class RevisionTask : BaseTranscriptionTask {

		public RevisionTask(IConfiguration cfg, IEnumerable<Claim> claims) : base(cfg, claims) { }

		public RevisionTask(IConfiguration cfg, IEnumerable<Claim> claims, long id) : base(cfg, claims, id) { }

		public RevisionTask(IConfiguration cfg, IEnumerable<Claim> claims, long sessionId, long stepId) : base(cfg, claims, sessionId, stepId) { }

		public RevisionTask(IConfiguration cfg, IEnumerable<Claim> claims, long sessionId, long stepId, long mark) : base(cfg, claims, sessionId, stepId, mark) { }
		...
		public override bool BeforeExecute() {
			...
			if (GetText == null) {
				...
				if (BLL.Cnn(cfg).ExecScalar<int?>(sql, new { sessionId, StepIdPrior }) == 1)
					throw new ValidationException("Revisão não permitida pois etapa anterior foi executada no ambiente desktop.");
			}
			return true;
		}
