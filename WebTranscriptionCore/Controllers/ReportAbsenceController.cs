...
using WebTranscriptionCore.Models;
using X.PagedList;
using static WebTranscriptionCore.OnlineProvider;

namespace WebTranscriptionCore.Controllers
{
	public class ReportAbsenceController : Controller
	{
		private readonly IConfiguration cfg;

		public ReportAbsenceController(IConfiguration cfg, IWebHostEnvironment env) => this.cfg = cfg;
		...
		{
			ReportAbsence st = new ReportAbsence(cfg);
			ReportAbsenceViewModel model = new ReportAbsenceViewModel();
			...
			model.ReportList = new StaticPagedList<ReportAbsence>(model.ReportList, page ?? 1, BLL.PageSize(cfg), pageCount);

			model.DateBeginSessionFilter = _DateBeginSessionFilter;
			model.DateEndSessionFilter = _DateEndSessionFilter;
			model.UserFilterListFilter = new List<IdName>() { new IdName() { Id = -1, Name = "Todos" } }.Concat(UserK.ListAll(cfg, false));
		...
		[HttpGet]
		[Authorize(Roles = "1")]
		public ActionResult SelectAllDropDown(string? id)
		{			
			return Json("");
		}
	}
