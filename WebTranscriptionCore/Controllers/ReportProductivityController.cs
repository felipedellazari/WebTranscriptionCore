...
{
	public class ReportProductivityController : Controller
	{
		private readonly IConfiguration cfg;

		public ReportProductivityController(IConfiguration cfg, IWebHostEnvironment env) => this.cfg = cfg;
		...
		{
			ReportProductivity st = new ReportProductivity(cfg);
			ReportProductivityViewModel model = new ReportProductivityViewModel();
			...
			model.ReportList = new StaticPagedList<ReportProductivity>(model.ReportList, page ?? 1, BLL.PageSize(cfg), pageCount);


			model.DateBeginSessionFilter = _DateBeginSessionFilter;
			model.DateEndSessionFilter = _DateEndSessionFilter;
			model.UserFilterListFilter = new List<IdName>().Concat(UserK.ListAll(cfg));
